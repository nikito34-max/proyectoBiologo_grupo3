...
import pkBioAnalytics.pkReinoViviente.pkProtista.Dinidium;
import pkBioAnalytics.pkReinoViviente.pkProtista.Heliozoo;
import pkBioAnalytics.pkReinoViviente.pkProtista.Paramecium;
...
public class MicroBiologo extends Persona {
    ...
    public void observar(List<Protozoo> protozoos) {
        ...
        for (Protozoo protozoo : protozoos) {
            ...
            if (protozoo instanceof Dinidium d) {
                d.localizar(parameciumEncontrado);
                d.inmovilizar (true);
                d.cazar (parameciumEncontrado);
                d.ingerir (parameciumEncontrado);
                d.crecer (6);
                dinidiumEncontrada = d;
            } else if (protozoo instanceof Paramecium p) {
                p.crecer(0.25);
                p.serAtacado(dinidiumEncontrada);
                parameciumEncontrado = p; 
            }
        }
